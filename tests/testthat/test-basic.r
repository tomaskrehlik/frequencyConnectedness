...
for (i in estimates) {
	context(sprintf("Sanity checks on estimate %s.", "a"))
	test_that("The BK absolute spillovers tables reconstruct perfectly in both cases", {
		...
		H <- 200
		expect_equivalent(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F)$tables[[1]], Reduce(`+`, frequencyConnectedness::spilloverBK09(i, n.ahead = H, partition = bounds, no.corr=F)$tables))
		expect_equivalent(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F)$tables[[1]], Reduce(`+`, frequencyConnectedness::spilloverBK12(i, n.ahead = H, partition = bounds, no.corr=F)$tables))
	})
	...
	test_that("The BK absolute spillovers reconstruct perfectly in both cases", {
		...
		bounds <- c(pi + 0.001, 2.5530220, 1.8705131, 0.6906649, 0.4298473, 0)
		H <- 200
		expect_equivalent(frequencyConnectedness::overall(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F))[[1]], Reduce('+', frequencyConnectedness::overall(frequencyConnectedness::spilloverBK09(i, n.ahead = H, partition = bounds, no.corr=F), within = F)))
		expect_equivalent(frequencyConnectedness::overall(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F))[[1]], Reduce('+', frequencyConnectedness::overall(frequencyConnectedness::spilloverBK12(i, n.ahead = H, partition = bounds, no.corr=F), within = F)))
	})
	...
	test_that("Test that spillovers are smaller than hundred", {
		...
		expect_that(frequencyConnectedness::overall(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F)) < 100, is_true())
		expect_that(frequencyConnectedness::overall(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F)) < 100, is_true())
	})
	...
	test_that("Test that TO spillovers are smaller than hundred", {
		...
		expect_that(all(frequencyConnectedness::to(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F))[[1]] < 100), is_true())
		expect_that(all(frequencyConnectedness::to(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F))[[1]] < 100), is_true())
	})
	...
	test_that("Test that TO spillovers sum up to the overall", {
		...
		expect_equivalent(sum(frequencyConnectedness::to(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F))[[1]]), frequencyConnectedness::overall(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F))[[1]])
		expect_equivalent(sum(frequencyConnectedness::to(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F))[[1]]), frequencyConnectedness::overall(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F))[[1]])
	})
	...
	test_that("Test that FROM spillovers sum up to the overall", {
		...
		expect_equivalent(sum(frequencyConnectedness::from(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F))[[1]]), frequencyConnectedness::overall(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=F))[[1]])
		expect_equivalent(sum(frequencyConnectedness::from(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F))[[1]]), frequencyConnectedness::overall(frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=F))[[1]])
	})
	...
	test_that("Test that for no.corr=T DY09 == DY12.", {
		...
		expect_equivalent(frequencyConnectedness::spilloverDY09(i, n.ahead = H, no.corr=T), frequencyConnectedness::spilloverDY12(i, n.ahead = H, no.corr=T))
	})
